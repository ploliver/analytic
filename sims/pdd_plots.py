...
if __name__=='__main__':
    ...
    t3c=Table.read('3crr_reduced.txt',format='ascii')

    # Convert each column from string to float
    for colname in t3c.colnames:
         # Replace non-numeric values (e.g., '--') with NaN
        t3c[colname] = np.where(t3c[colname] == '--', np.nan, t3c[colname])
        t3c[colname] = t3c[colname].astype(float)
    ...
    g=glob.glob("pdd/"+names[plotnumber])
    num_elements = len(g)
    actual = 0

    for f in g:
        actual = actual+1
        print(f"Starting with glob number {actual}/{num_elements}")
        sdict={}
...
#        ax.set_xlabel('R/kpc')
    ...
    for fig in range(4):
        filename="figures/"+prefixes[plotnumber]+figs[fig]
        plt.figure(fig+1)
